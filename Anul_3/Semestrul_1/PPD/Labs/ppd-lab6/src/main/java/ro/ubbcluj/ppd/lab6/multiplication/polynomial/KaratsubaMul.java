...
public class KaratsubaMul {
    ...
    private static int[] internalSequentialMultiplication(int a[], int b[]) {
        if (a.length < 2 || b.length < 2) {
            return SimpleMul.sequentialMultiplication(a, b);
        }
    ...
    private static int[] internalParallelMultiplication(int a[], int b[], AtomicInteger numberOfRemainingThreads) {
        ...
        if (numberOfRemainingThreads.get() > 0) {
            ...
            pf = new FutureTask<>(() -> internalParallelMultiplication(lowA, lowB, numberOfRemainingThreads));
        ...
        if (numberOfRemainingThreads.get() > 0) {
            numberOfRemainingThreads.decrementAndGet();
            qf = new FutureTask<>(() -> internalParallelMultiplication(polyAdd(lowA, highA), polyAdd(lowB, highB), numberOfRemainingThreads));
        ...
        if (numberOfRemainingThreads.get() > 0) {
            numberOfRemainingThreads.decrementAndGet();
            rf = new FutureTask<>(() -> internalParallelMultiplication(highA, highB, numberOfRemainingThreads));
