 ...
 */
public class KaratsubaNumberMultiplication {
    private KaratsubaNumberMultiplication() {}
    ...
    private static int[] internalSequentialMultiplication(int a[], int b[]) {
        if (a.length < 2 || b.length < 2) {
            return SimpleNumberMultiplication.sequentialMultiplication(a, b);
        }
        ...
        int p[] = internalSequentialMultiplication(lowA, lowB);
        int q[] = internalSequentialMultiplication(numberAdd(lowA, highA), numberAdd(lowB, highB));
        int r[] = internalSequentialMultiplication(highA, highB);

        return numberAdd(numberAdd(numberShift(r, 2 * halfPoint), numberShift(numberSub(numberSub(q, r), p), halfPoint)), p);
    }
    ...
    private static int[] internalParallelMultiplication(int a[], int b[], AtomicInteger numberOfRemainingThreads) {
        ...
        if (numberOfRemainingThreads.get() > 0) {
            ...
            qf = new FutureTask<int[]>(new Callable<int[]>() {
                @Override
                public int[] call() throws Exception {
                    return internalParallelMultiplication(numberAdd(lowA, highA), numberAdd(lowB, highB), numberOfRemainingThreads);
                }
            });
        ...
        if (qf == null) {
            q = internalParallelMultiplication(numberAdd(lowA, highA), numberAdd(lowB, highB), numberOfRemainingThreads);
        }
        if (rf == null) {
    ...
    private static int[] numberAdd(int a[], int b[]){
    ...
    private static int[] numberShift(int a[], int positions) {
    ...
    private static int[] numberSub(int a[], int b[]) {
        int limit = Math.max(a.length, b.length);
        int result[];
        ...
        return Utils.noLeadingZeros(Utils.moveNegativeCarries(result));
    }
