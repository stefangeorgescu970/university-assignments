...
public class DirectorySelectPanel extends JPanel {
    ...
    /**
     * Sets reference to the MGUI object that instantiated this object to signal changes.
     * @param delegate the MGUI
     */
    ...
    public DirectorySelectPanel(){
        ...
        this.directoryLabel = new JLabel("Directory " + DirectorySelectPanel.instanceId++ + ":");
        this.directoryLabel.setFont(GUIConstants.appFont);
        this.directoryLabel.setPreferredSize(GUIConstants.prePathLabelDimension);
        ...
        this.selectOrChangeButton.setPreferredSize(GUIConstants.buttonDimension);
        this.selectOrChangeButton.setMinimumSize(GUIConstants.buttonDimension);
        constraints.gridx = 6;
        constraints.gridwidth = 1;
        this.add(this.selectOrChangeButton, constraints);

        this.selectOrChangeButton.addActionListener(e -> selectOrChangeButtonPressed());
        ...
        this.warningLabel = new JLabel("Warning! This directory has no images in it.");
    ...
    void selectOrChangeButtonPressed(){
        ...
        /**
         * Make sure that the user knows what he is doing, and only after that discard merge results
         */
        int result = JOptionPane.NO_OPTION;
        if(this.delegate.userIsViewingMergeResults()) {
            result = JOptionPane.showConfirmDialog(null, "All merge progress will be lost. Do you wish to continue?", "Warning",
                    JOptionPane.YES_NO_OPTION);
        }

        if(!this.delegate.userIsViewingMergeResults() || result == JOptionPane.YES_OPTION) {
            ...
            if(System.getProperty("os.name").toLowerCase().contains("mac")){
                ...
                FileDialog fd = new FileDialog(frame, "Choose a directory", FileDialog.LOAD);
                fd.setDirectory("");
                fd.setVisible(true);
                ...
                if (chooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) {
                    pathName = chooser.getCurrentDirectory().toString() + chooser.getSelectedFile().toString();
                }
            }
            ...
            if(pathName != null) {
                ...
                this.delegate.filePathChanged(myDir, this.directoryLabel.getText());
     ...
     * @param pathName the path of the current directory
     */
    public void setFilePath(String pathName) {
        this.pathLabel.setText("..." + pathName.substring(pathName.length() / 2 , pathName.length()));
    }
